...
class Wrapper(object):
    ...
    def __init__(self):
        ...
        counts = self.db.execQuery(
            """select h.id, count(h2.id)
            from heading h
        ...
        if lang == "fr":
            if pos is None:
                ...
                results = self.db.execQuery(
                    """
                    select id from word
                    where fr_translation=%s and pos=%s""",
                    (strWord, pos),
                )
    ...
    def getTierIndexChildren(self, heading_id):
        ...
        if root == "root":
            ...
            if parent_id == "root":
                ...
                h = self.db.execQuery(
                    "select fr_heading from heading where id=%s", (parent_id,)
                )
                heading_name = h[0][0]
            ...
            # then add in us
            new_line = self.getCSVLine(
                heading_id, root_heading.fr, parent_id, cat=root_cat
            )
            line_list.append(new_line)
        ...
        if root != "root":
            csv += self.getHeadingChildrenCSVList(
                root, heading_id, parent_list
            )  # now append all children
        return csv
    ...
    def getCSVLine(
        ...
        if (set_size == "" or child_size == "") and heading_id != "root":
            result = self.db.execQuery(
                "select size, pos_size from wordsize where headingid=%s", (heading_id,)
            )
            set_size = result[0][1]
    ...
    def getHeadingChildrenCSVList(
        self, root=None, heading_id=None, parent_list={}, output_csv=True
    ):
        ...
        if not recovering:
            ...
            for term in term_list[base_word]:
                ...
                for key in term_list:
                    ...
                    if key == base_word:
                        continue
                    heading, score = self.getClosestHeading(
                        term["tier_index"], term_list[key]
                    )
                    temp.append(heading)
                    total_score += score
